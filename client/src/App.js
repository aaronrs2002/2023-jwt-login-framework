...
function App() {
  ...
  const createUser = () => {
    ...
    axios.post("/newUser",
      { theme: "css/bootstrap.min.css", "email": email, "level": level, "password": password },
      {
        headers: {
          "Content-Type": "application/json"
        }
  ...
  const logout = () => {
    ...
    axios.put("/logout-uuid", {
      email: userEmail,
      uuid: "logged-out"/*generate-server-side-uid new feature 2023-07-08*/
    }).then(
      (res) => {
            ...
            <div className="row">
              <div className="col-md-3">
                {infoMessage === "account-settings" ?
                  <a href="#settingsPanel" className="btn btn-secondary w-100 btn-block" onClick={() => setInfoMessage((infoMessage) => "")} >{userEmail} <i className="fas fa-cog"></i></a> :
                  <a href="#settingsPanel" className="btn btn-secondary w-100 btn-block" onClick={() => setInfoMessage((infoMessage) => "account-settings")} >{userEmail} <i className="fas fa-cog"></i></a>}
                {infoMessage === "account-settings" ?
              ...
              <div className="col-md-2">
                <button className="btn btn-block w-100 btn-danger" onClick={() => logout()}>Logout</button>
              </div>
            </div>
          </footer>
        </React.Fragment>
